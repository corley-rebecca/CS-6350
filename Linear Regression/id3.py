####################################################
##### This code is for ML 6350 HW2 - Problem 2 #####
####################################################
'''
1. Modify to learn decision stump (trees with two levels) - compute IG to select best feature to split on
2. For each subset, create a leaf node
3. Based on decision stump learning algorithm, implement AdaBoost
'''
...
def get_best_split_attr(examples, attrs, labels, split_type):
    ...
    return split_attr
# decision tree - give depth 2 to create stump
def build_dtree(examples, attrs, labels, attr_order, depth=2, gain_type="IG"):
